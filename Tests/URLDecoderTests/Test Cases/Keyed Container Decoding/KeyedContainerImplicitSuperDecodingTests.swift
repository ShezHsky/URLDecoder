...
class KeyedContainerImplicitSuperDecodingTests: XCTestCase {
    ...
    func testSuperDecoderInheritsCodingPath() throws {
        ...
        let decoder = URLDecoder()
        let decoded = try decoder.decode(Child.self, from: url)
        let codingPath = try XCTUnwrap(decoded.superDecoder?.codingPath as? [URLCodingKeys])
        let expected = [URLCodingKeys.super]
